...
## 扩展：忽略 no significant change 快捷按钮 + 快捷时间段

### 新增查询参数

- `hide_no_significant_change`：为 `true` 时隐藏 `skip_reason=no significant change` 的跳过记录。
  与 `hide_symbol_disabled` 可同时启用，分别生效。

### 跳过原因集中管理

`src/throttle/gate.py` 暴露 `NO_SIGNIFICANT_CHANGE_REASON` 常量，
`web/status.py` 与节流层共用同一字符串，避免拼写漂移。

### 时间筛选升级

- 保留原 `el-date-picker` 自由范围。
- 新增快捷时间段按钮组（`QUICK_TIME_RANGES`），含 `1h / 3h / 12h / 1d / 7d / 30d`。
- 选中快捷时段后，`el-date-picker` 自动禁用并清空。
- 选中自由范围后，自动清除快捷时段选中。
- 提供 “清除时间” 按钮统一清空时间筛选。

### 行为细节

- 快捷时段与币种/类型/忽略按钮正交，可同时生效。
- 修改任何筛选都会触发 `search()` 并重置分页到第一页，与现有交互一致。
- 快捷时段在选择瞬间立即计算 `now` 作为终点，避免依赖时钟漂移。

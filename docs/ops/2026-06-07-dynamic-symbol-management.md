...
交易主进程的周期性对账会检查交易所持仓和保护条件单。

旧逻辑中，只要某个 active 注册币种在交易所存在持仓，但没有 active SL 条件单，
系统就会认为该持仓没有止损保护，并触发应急平仓。

这对系统自己开的仓是合理的，但对以下场景不合适：

- 用户在交易所控制台手动开的仓。
- 动态新增币种时发现的已有持仓。
- `enabled=false` 的币种上存在人工持仓。
- 尚未执行“接管已有持仓”流程的持仓。

因此保护单不变量检查增加所有权边界：

```text
只有同时满足以下条件，才允许自动撤陈旧条件单或应急平仓：

1. symbols.enabled = true
2. 本地 trades 表存在该币种 open/partial 交易组
```
...
如果交易所存在持仓，币种启用但本地没有 open 交易组：

- 认为这是未接管持仓。
- 自动把该币种设为 `enabled=false`。
- 发送错误告警。
- 不自动撤单。
- 不自动平仓。

...
私有流确认任一侧 SL/TP 条件单 `FILLED` 后，engine 会立即调用：

```text
cancel_all_condition_orders(symbol)
```

清理该交易对剩余条件单。正常情况下，被触发的一侧已经不在 open algo 列表里；
仍存在的通常是同仓位另一侧 TP/SL。撤销后 engine 会回查交易所 open 条件单：

- 已消失的条件单同步为本地 `canceled`
- 仍残留的条件单同步为本地 `placed`
- 若残留未能撤掉，则禁用该 symbol，并标记为 `STALE_CONDITION_ORDERS`

这使「盈利触发 SL」和「保本触发 SL」在交易所侧具有相同清理行为：触发后不会让同仓位 TP 长时间裸挂。
...
- 如果私有流或撤单 REST 临时失败，周期对账仍会再次清理无仓位残留条件单；若残留持续存在，会禁用币种进入人工处理。

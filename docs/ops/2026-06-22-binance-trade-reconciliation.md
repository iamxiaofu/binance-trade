...
- `当日风控回撤`：当前账户权益相对本地自然日内最高权益的回撤；这是
  `max_drawdown_pct` 熔断使用的指标；
- `账户历史回撤`：当前账户权益相对持久化历史最高权益的回撤，仅用于绩效展示
  和审计，不再直接触发熔断；
...
风控周期按本地自然日计算。跨日后，以当日首个有效账户权益作为新周期基准，之后持续
抬高当日权益峰值。若当日回撤触发熔断，用户可以通过主网高风险确认人工恢复；人工恢复
会只豁免当天剩余时间的“回撤熔断”，当日亏损熔断、私有流和其他安全检查仍然有效。
次日自动清除豁免并恢复回撤保护。豁免日期和时间写入运行时设置与控制命令审计。
